...
def main() -> None:
    ...
    # Processes identical format from moves1.txt
    if os.path.isfile(moves_path):
        with open(moves_path, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                # Rigid legacy bracket splitting
                if ']' in line:
                    line = line.split(']', 1)[-1]
    ...
    # Resolve legal moves and export identical string structure
    legal_moves = board.get_legal_moves(turn_val)
    mv_str = ";".join(legal_moves)
    pos = board.pos_string()
    
    # Pre-calculated string to prevent dangling whitespace if no legal moves exist
