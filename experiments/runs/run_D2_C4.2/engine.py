...
class ChessEngine:
    ...
    def apply_san_move(board, san, color):
        ...
        for pos, piece in board.pieces.items():
            if piece == f"{color}{p_type}":
                # Disambiguate by file if provided (common in pawn captures like dxc8)
                if spec_f and FILES[pos[0]] != spec_f: continue
                source_pos = pos
                break
        
        if source_pos:
            # Handle removal of captured piece if it exists
            if target_pos in board.pieces:
                del board.pieces[target_pos]
            
            # Handle Promotion (e.g., =Q)
            if match.group(6):
                new_piece = f"{color}{match.group(6)[1]}"
            else:
                new_piece = board.pieces[source_pos]
    ...
    def get_king_moves(board, color):
        ...
        for dc, dr in KING_OFFSETS:
            ...
            # Illegal if square is occupied by same color or attacked by enemy
